 ...
 * @package    EcomCore_Freight
 */
class EcomCore_Freight_Model_Shipping_Carrier_Australiapost
    extends Mage_Shipping_Model_Carrier_Abstract
    implements Mage_Shipping_Model_Carrier_Interface
{
    const EXTRA_COVER_LIMIT = 5000;

    protected $_code = 'eccfreightpost';
    ...
    public function __construct()
    {
        ...
        if ($helper->isAustraliaPostDeveloperMode()) {
            ...
            Mage::log('You need a valid API key in order to use this feature.', null, 'ecomcore_freight.log');
        }
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        if ($origCountry != EcomCore_Freight_Helper_Data::AUSTRALIA_COUNTRY_CODE) {
            return false;
        }
        ...
        if (!$destCountry) {
            $destCountry = EcomCore_Freight_Helper_Data::AUSTRALIA_COUNTRY_CODE;
        }
    ...
    protected function _getQuotes($extraCover, $config)
    {
        ...
        try {
            if ($destCountry == EcomCore_Freight_Helper_Data::AUSTRALIA_COUNTRY_CODE) {
            ...
            if (in_array($serviceCode, $allowedMethods)) {
                ...
                if (
                    ...
                    foreach ($serviceOption as $option) {
                        ...
                        /** @var EcomCore_Freight_Helper_Clickandsend $clickandsendHelper */
                        $clickandsendHelper = Mage::helper('eccfreight/clickandsend');
    ...
    {
        $suboptions = $this->_getOptionVisibilities($destCountry, EcomCore_Freight_Model_Shipping_Carrier_Australiapost_Source_Visibility::NEVER);
    ...
    {
        $suboptions = $this->_getOptionVisibilities($destCountry, EcomCore_Freight_Model_Shipping_Carrier_Australiapost_Source_Visibility::REQUIRED);
        foreach ($suboptions as $suboption) {
            if (stripos($name, $suboption) === false) {
                return false;
            }
    ...
    protected function _getOptionVisibilities($destCountry, $visibility)
    {
        ...
        $helper = Mage::helper('eccfreight/eccfreightpost');
        $suboptions = array();
        if ($helper->getPickUp() == $visibility && $destCountry != EcomCore_Freight_Helper_Data::AUSTRALIA_COUNTRY_CODE) {
            $suboptions[] = 'pick up';
        }
        if ($helper->getExtraCover() == $visibility) {
            $suboptions[] = 'extra cover';
        }
        if ($helper->getSignatureOnDelivery() == $visibility && $destCountry == EcomCore_Freight_Helper_Data::AUSTRALIA_COUNTRY_CODE) {
    ...
    {
        /** @var EcomCore_Freight_Helper_Data $helper */
