...
class EcomCore_Freight_Model_Observer
{
    /**
    ...
    public function addHistory($event)
    {
        ...
        if ($order && $order->getPayment()) {
            if ($order->getPayment()->getMethod() == EcomCore_Freight_Model_Payment_Directdeposit::CODE) {
                $order->addStatusHistoryComment('Order placed with Direct Deposit')->setIsCustomerNotified(false);
            } elseif ($order->getPayment()->getMethod() == EcomCore_Freight_Model_Payment_Bpay::CODE) {
    ...
    {
        if (!Mage::helper('eccfreight/address')->isAddressValidationEnabled()) {
            return;
        }
